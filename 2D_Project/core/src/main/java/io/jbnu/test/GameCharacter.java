...
import io.jbnu.test.Moving.Dash;
...
public class GameCharacter {
    ...
    // --- 3. 행동 (메서드) ---
    public void moveRight() {                   //우이동
        if (velocity.x < 800f) {
            velocity.x += 100f;
        }
    }
    public void moveLeft() {                    //좌이동
        if (velocity.x> (-800f)){
            velocity.x += -100f;
        }
    }
    //-------------------대쉬--------------------
    public void dash(Vector2 direction){        //대쉬
        dash.startDash(direction,isGrounded);
    }
    public boolean isDashing() {                //대쉬중인가
        return dash.isDashing();
    }
    public void canDash() {                   //바닥에 착지했는가
        dash.canDash();
    }
    public void updateDash(float delta) {     //대쉬 적용(업데이트)
        dash.update(delta, velocity);
